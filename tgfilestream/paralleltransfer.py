...
class DCConnectionManager:
    ...
    def __init__(self, client: TelegramClient, dc_id: int) -> None:
        ...
        self.auth_key = None
        self.connections = []
        self._list_lock = asyncio.Lock()  # Remove 'loop=self.loop'
    ...
    async def _new_connection(self) -> Connection:
        ...
        conn = Connection(sender=sender, log=self.log.getChild(f"conn{index}"), lock=asyncio.Lock())  # Remove 'loop=self.loop'
